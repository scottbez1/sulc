...
/*
This is a modified version of Tlc5940.cpp from the Arduino TLC5940 Library,
for use with SULC.
*/
...
//#else
    TCCR0A = _BV(COM0B1)      // set on BOTTOM, clear on OCR2A (non-inverting),
                              // output on OC2B
           | _BV(WGM01)       // Fast pwm with OCR2A top
           | _BV(WGM00);      // Fast pwm with OCR2A top
    TCCR0B = _BV(WGM02);      // Fast pwm with OCR2A top
